...
type DefaultMapper<'TSource, 'TDestination when 'TSource : equality and 'TSource : not struct and 'TDestination : equality and 'TDestination : not struct>() = 
    let (|NoValue|Value|) s =
        if s = Unchecked.defaultof<'TSource> then
            NoValue
        else
            Value
    ...
    override this.Map(source) = 
        match source with
        | NoValue -> Unchecked.defaultof<'TDestination>
        | Value -> 
